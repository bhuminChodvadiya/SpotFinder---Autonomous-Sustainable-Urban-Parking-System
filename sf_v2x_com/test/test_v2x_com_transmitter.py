...
class TestTransmitter(unittest.TestCase):
    ...
    def test_ego_position_callback(self):
        ...
        ego_position_msg.longitude = 10
        ego_position_msg.latitude = 20
        ego_position_msg.yaw_angle = 30
        ego_position_msg.velocity = 10
        ego_position_msg.acceleration = 2

        self.transmitter.ego_position_callback(ego_position_msg)
    ...
    def test_detectnet_callback_with_detections(self):
        detection_msg = Detection2DArray()
        # Assuming you populate detection_msg with some detection data
        # For simplicity, let's simulate 2 detections
        ...
        for perceived_object in cpm_msg.payload.cpm_containers.perceived_object_container.perceived_objects:
            self.assertIn(perceived_object.object_id, [0, 1])  # Assuming detection IDs start from 0 or 1
